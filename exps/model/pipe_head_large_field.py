...
class PIPEHeadLargeField(nn.Module):
    def __init__(
        self,
        num_classes,
        width=1.0,
        ...
        act="silu",
        depthwise=False,
        stem_layer_nums=[1, 1, 1], # The stem layer num for each head.
        stem_kernel_sizes=[7, 7, 7]
    ):
        """
        Args:
        ...
        """
        super().__init__()
        assert (len(in_channels) == len(stem_layer_nums) == len(stem_kernel_sizes), 
            "The length of in_channels, stem_layer_nums and stem_kernel_sizes must be equal.")
        for i in range(len(stem_layer_nums)):
            assert stem_layer_nums[i] > 0, "stem_layer_num must be larger than 0."
        ...
        for i in range(len(in_channels)):
            stem_layer_num = stem_layer_nums[i]
            stem_layers = [
                BaseConv(
                    in_channels=int(in_channels[i] * width),
                    out_channels=int(256 * width),
                    ksize=stem_kernel_sizes[i],
                    stride=1,
                    act=act,
                )
            ]
            for _ in range(stem_layer_num-1):
                stem_layers.append(
                        BaseConv(
