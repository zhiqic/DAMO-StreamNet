...
class DFPPAFPNSHORTV3_TS(nn.Module):
    """
    ...
    def __init__(
        ...
        time_speed_cfg=dict(
                ...
                extra_conv_out_channels=[((64, 128, 256), 1), ],
            ),
    ):
        super().__init__()
        ...
        self.use_concat = time_speed_cfg["use_concat"]
        self.use_extra_conv = time_speed_cfg["use_extra_conv"]
        self.extra_conv_out_channels = time_speed_cfg["extra_conv_out_channels"]
        self.extra_conv_group_num = len(time_speed_cfg["extra_conv_out_channels"])
        assert self.conv_group_num == self.extra_conv_group_num
        ...
        for i in range(self.extra_conv_group_num):
            setattr(self,
                    f"extra_group_{i}_jian2",
                    Conv(
                        in_channels=int(in_channels[0] * width),
                        out_channels=self.extra_conv_out_channels[i][0][0],
                        ksize=1,
                        stride=1,
                        act=act,
                    )
            ...
            setattr(self,
                    f"extra_group_{i}_jian1",
                    Conv(
                        in_channels=int(in_channels[1] * width),
                        out_channels=self.extra_conv_out_channels[i][0][1],
                        ksize=1,
                        stride=1,
                        act=act,
                    )
            ...
            setattr(self,
                    f"extra_group_{i}_jian0",
                    Conv(
                        in_channels=int(in_channels[2] * width),
                        out_channels=self.extra_conv_out_channels[i][0][2],
                        ksize=1,
                        stride=1,
                        act=act,
                    )
    ...
    def off_forward(self, input, backbone_neck):
        ...
        for i in range(self.conv_group_num):
            ...
            for j in range(group_frame_num):
                ...
                if self.use_extra_conv:
                    tmp_tensor2 = getattr(self, f"extra_group_{i}_jian2")(all_pan_out2s[frame_id])
                    tmp_tensor1 = getattr(self, f"extra_group_{i}_jian1")(all_pan_out1s[frame_id])
                    tmp_tensor0 = getattr(self, f"extra_group_{i}_jian0")(all_pan_out0s[frame_id])
        ...
        if not self.use_concat:
            ...
            if self.with_short_cut:
                pan_out2 = torch.cat(pan_out2s + pan_out2s, dim=1) + rurrent_pan_out2
                pan_out1 = torch.cat(pan_out1s + pan_out1s, dim=1) + rurrent_pan_out1
                pan_out0 = torch.cat(pan_out0s + pan_out0s, dim=1) + rurrent_pan_out0
            else:
                pan_out2 = torch.cat(pan_out2s + pan_out2s, dim=1)
                pan_out1 = torch.cat(pan_out1s + pan_out1s, dim=1)
                pan_out0 = torch.cat(pan_out0s + pan_out0s, dim=1)
