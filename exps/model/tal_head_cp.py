...
class TALHeadCP(nn.Module):
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            cls_feat = cp(cls_conv, cls_x)
            cls_output = cp(self.cls_preds[k], cls_feat)

            reg_feat = cp(reg_conv, reg_x)
            reg_output = cp(self.reg_preds[k], reg_feat)
            obj_output = cp(self.obj_preds[k], reg_feat)
