...
class MosaicDetectionDil(Dataset):
    """Detection dataset wrapper that performs mixup for normal dataset."""
    ...
    def __getitem__(self, idx):
        if self.enable_mosaic and random.random() < self.mosaic_prob:
            ...
            self._dataset._input_dim = self.input_dim
            img, support_img, future_img, label, support_label, img_info, id_ = self._dataset.pull_item(idx)
            img, support_img, future_img, label, support_label = self.preproc((img, support_img, future_img), (label, support_label), self.input_dim)
            return np.concatenate((img, support_img, future_img), axis=0), (label, support_label), img_info, id_


class LongShortMosaicDetectionDil(Dataset):
    """Detection dataset wrapper that performs mixup for normal dataset."""
    ...
    def __getitem__(self, idx):
        if self.enable_mosaic and random.random() < self.mosaic_prob:
            ...
            self._dataset._input_dim = self.input_dim
            short_imgs, long_imgs, future_img, label, support_label, img_info, id_ = self._dataset.pull_item(idx)
            short_imgs, long_imgs, future_img, label, support_label = self.preproc(short_imgs, long_imgs, future_img, (label, support_label), self.input_dim)
            # return np.concatenate((img, support_img), axis=0), (label, support_label), img_info, id_
            if len(long_imgs) > 0:
                return np.concatenate(short_imgs+[future_img], axis=0), np.concatenate(long_imgs, axis=0), (label, support_label), img_info, id_
            else: # 不使用long支路的情况
                return np.concatenate(short_imgs+[future_img], axis=0), np.zeros((0, )), (label, support_label), img_info, id_
            # return np.zeros(np.concatenate(short_imgs, axis=0).shape, dtype=np.float32), np.ones(np.concatenate(long_imgs, axis=0).shape, dtype=np.float32), (label, support_label), img_info, id_


class LongShortMosaicNTargetDetection(Dataset):
    """Detection dataset wrapper that performs mixup for normal dataset."""
    ...
    def __getitem__(self, idx):
        if self.enable_mosaic and random.random() < self.mosaic_prob:
            ...
            self._dataset._input_dim = self.input_dim
            short_imgs, long_imgs, label, support_label, img_info, id_, ntarget = self._dataset.pull_item(idx)
            short_imgs, long_imgs, label, support_label, ntarget = self.preproc(short_imgs, long_imgs, (label, support_label, ntarget), self.input_dim)
            # return np.concatenate((img, support_img), axis=0), (label, support_label), img_info, id_
            if len(long_imgs) > 0:
                return np.concatenate(short_imgs, axis=0), np.concatenate(long_imgs, axis=0), (label, support_label, ntarget), img_info, id_
            else: # 不使用long支路的情况
                return np.concatenate(short_imgs, axis=0), np.zeros((0, )), (label, support_label, ntarget), img_info, id_
            # return np.zeros(np.concatenate(short_imgs, axis=0).shape, dtype=np.float32), np.ones(np.concatenate(long_imgs, axis=0).shape, dtype=np.float32), (label, support_label), img_info, id_
