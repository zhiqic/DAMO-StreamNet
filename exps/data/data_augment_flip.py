...
class DoubleTrainTransformDil:
    ...
    def __call__(self, image, targets, input_dim):
        ...
        img1, label1 = self.trasform1(image[0], targets[0], input_dim, mirror=a)
        img2, label2 = self.trasform2(image[1], targets[1], input_dim, mirror=a)
        # t+1没有对应的targets需要处理，这里使用targets[1]，仅为了方便调用self.trasform3函数
...
class LongShortTrainTransformDil:
    def __init__(self, max_labels=50, hsv=True, flip=True, short_frame_num=2, long_frame_num=2):
        self.max_labels = max_labels
        self.short_frame_num = short_frame_num
        self.long_frame_num = long_frame_num
        self.short_transforms = [TrainTransform(max_labels=max_labels, hsv=hsv, flip=flip) for _ in range(short_frame_num)]
        self.long_transforms = [TrainTransform(max_labels=max_labels, hsv=hsv, flip=flip) for _ in range(long_frame_num)]
        self.trasform = TrainTransform(max_labels=max_labels, hsv=hsv, flip=flip)

    def __call__(self, short_images, long_images, future_image, targets, input_dim):
        a = random.randrange(2)
        short_res = []
        long_res = []
        ...
        future_img, label = self.trasform(future_image, targets[1], input_dim, mirror=a)

        return short_imgs, long_imgs, future_img, short_res[0][1], short_res[1][1] if self.short_frame_num > 1 else support_res[1]

class LongShortTrainNTaretTransform:
    """
        Refer to LongShortTrainTransform, targets has three elements: (target, support_target, ntarget)
    """
    def __init__(self, max_labels=50, hsv=True, flip=True, short_frame_num=2, long_frame_num=2):
        self.max_labels = max_labels
        self.short_frame_num = short_frame_num
    ...
    def __call__(self, short_images, long_images, targets, input_dim):
        ...
        ntarget_res = trans_func(short_images[0].copy(), targets[2], input_dim, mirror=a)
        ...
        return short_imgs, long_imgs, short_res[0][1], short_res[1][1] if self.short_frame_num > 1 else support_res[1], ntarget_res[1]
...
class LongShortValNTargetTransform:
    def __init__(self, swap=(2, 0, 1), short_frame_num=2, long_frame_num=2):
        self.short_frame_num = short_frame_num
        self.long_frame_num = long_frame_num
        self.short_transforms = [ValTransform(swap=swap) for _ in range(short_frame_num)]
    ...
    def __call__(self, short_images, long_images, res, input_size):
        ...
        ntarget_res = trans_func(short_images[0].copy(), res[2], input_size)
