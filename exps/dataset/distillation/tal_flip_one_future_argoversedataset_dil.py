...
class ONE_Dil_ARGOVERSEDataset(Dataset):
    """
    COCO dataset class.
    """
    ...
    def load_anno_from_ids(self, id_):
        ...
        ## back fid
            ...
            im_ann_future = self.coco.loadImgs(id_ + 1)[0]

        im_name_support = im_ann_support['name']
        im_sid_support = im_ann_support['sid']

        im_name_future = im_ann_future['name']
        im_sid_future = im_ann_future['sid']
        ...
        support_file_name = os.path.join(self.data_dir, 'Argoverse-1.1', 'tracking', self.seq_dirs[im_sid_support], im_name_support)
        future_file_name = os.path.join(self.data_dir, 'Argoverse-1.1', 'tracking', self.seq_dirs[im_sid_future], im_name_future)

        return (res, support_res, img_info, resized_info, file_name, support_file_name, future_file_name)
    ...
    def pull_item(self, index):
        ...
        return img, support_img, future_img, res.copy(), support_res.copy(), img_info, np.array([id_])
    ...
    def __getitem__(self, index):
        ...
        """
        img, support_img, future_img, target, support_target, img_info, img_id = self.pull_item(index)

        if self.preproc is not None:

            img, support_img, future_img, target, support_target = self.preproc((img, support_img, future_img), (target, support_target), self.input_dim)

        return np.concatenate((img, support_img, future_img), axis=0), (target, support_target), img_info, img_id
