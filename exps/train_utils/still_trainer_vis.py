...
class Trainer:
    def __init__(self, exp: Exp, args):
        ...
        self.best_ap = 0
        self.img_cnt = 0
        # self.img_root_dir = "/mnt/data3/lee.lcy/Datasets/opensource/Argoverse/output/stream_yolo/still_dfp_fix/s_s50_still_dfp_flip_ep8_vis/vis_images"
        self.img_root_dir = "/mnt/data3/lee.lcy/Datasets/opensource/Argoverse/output/stream_yolo/still_dfp_fix/s_s50_still_dfp_flip_ep8_no_fix_vis/vis_images"
    ...
    def train_one_iter(self):
        ...
        inps_np = np.transpose(inps.cpu().numpy().astype(np.uint8), (0, 2, 3, 1))
        targets_np = targets.cpu().numpy().astype(np.int32)
        img_vis = inps_np[0].copy()
        for anno in targets_np[0]:
            if anno[0] == 0:
                continue
            cx, cy, w, h = anno[1:]
            img_vis = cv2.rectangle(img_vis, (cx-w//2,cy-h//2), (cx+w//2,cy+h//2), (0,255,0), 2)

        img_path = os.path.join(self.img_root_dir, f'{self.img_cnt}.jpg')
